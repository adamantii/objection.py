...
class Asset:
  ...
  def _requestData(cls, *_) -> tuple[bool, dict]:
    raise NotImplementedError("_requestData must be implemented by " + cls.__name__ + " subclasses")
...
class Character(PostAsset):
  ...
  _assetKeys = ('alignment', 'backgroundId', 'blipUrl', 'bubbles', 'galleryAJImageUrl', 'galleryImageUrl', 'iconUrl', 'limitWidth', 'name', 'namePlate', 'offsetX', 'offsetY', 'poses', 'side')
  ...
  def lookupPoseSubstr(self, lookupSubstr: str) -> int:
    ...
    for i,key in enumerate(self._poseLookupKeys):
      if keyToFind not in key: continue
