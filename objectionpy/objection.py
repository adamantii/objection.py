...
class AssetBank:
  ...
  def loadAssets(self, assetDict: dict[str, assets.Asset]):
    for name, asset in assetDict.items():
      if type(asset) is assets.Character: self.characters[name] = asset
      elif type(asset) is assets.Background: self.backgrounds[name] = asset
      elif type(asset) is assets.Music: self.music[name] = asset
      elif type(asset) is assets.Sound: self.sounds[name] = asset
      elif type(asset) is assets.Popup: self.popups[name] = asset
      elif type(asset) is assets.Evidence: self.evidence[name] = asset
      else:
        raise TypeError('Unknown asset type ' + type(asset).__name__)
  
  def loadAssetIDs(self, assetType: type, ids: dict[str, int]):
    if assetType is assets.Character: targetDict = self.characters
    elif assetType is assets.Background: targetDict = self.backgrounds
    elif assetType is assets.Music: targetDict = self.music
    elif assetType is assets.Sound: targetDict = self.sounds
    elif assetType is assets.Popup: targetDict = self.popups
    elif assetType is assets.Evidence: targetDict = self.evidence
    else:
...
class Group:
  ...
  def _exceptIfNotCEType(self):
    if self.type is not enums.GroupType.CROSS_EXAMINATION:
      raise TypeError('counsel and failure frame sequences are only available in cross-examination groups')
...
class _ObjectionBase:
  ...
  def _compileFrame(self, frame: Frame, requestPair: _lru_cache_wrapper, frameList: list[Frame], frameTags: dict, groupTags: dict):    
    ...
    activeIndex = _utils._maxIndex([*map(lambda char: char._getIndividualValue(char.isActive), chars)])
    activeChar = chars[activeIndex]
    secondaryChar = chars[1 - activeIndex]
    frontChar = chars[_utils._maxIndex([*map(lambda char: char._getIndividualValue(char.isFront) if not char.isNone else -2, chars)])]
    ...
    if frame.options.galleryRemove is not None:
      for location in frame.options.galleryRemove: # TODO make it choose dynamically between action 3 (PW) and 11 (AJ) galleries
        frameObject['frameActions'].append({
          'actionId': 3,
    ...
    if pairNeeded:
      ...
      pairChars.sort(key = lambda char: char.character.id if char.character.id is not None else 0, reverse = True)
      pair = requestPair(pairChars[0].character.id, pairChars[1].character.id, pairChars[0].pairOffset, pairChars[1].pairOffset, True if chars[0] is frontChar else False)
      frameObject['pairId'] = pair['pairId']

    if frame.onCompile is not None:
      frameObject = frame.onCompile(frameObject)

    LimitWarning.checkList(frameObject['frameActions'], self.options.MAX_FRAME_ACTIONS, 'frame actions (frame iid=' + str(frameObject['iid']) + ')')
  ...
  def compile(self) -> dict:
    ...
    LimitWarning.checkList(objectionObject['aliases'], self.options.MAX_ALIASES, 'aliases')
    ...
    for i,group in enumerate(self._groups):
      ...
      for frame in group.frames:
        frameObject = self._compileFrame(frame, requestPair = requestPair, frameList = groupObject['frames'], frameTags = frameTags, groupTags = groupTags)
      ...
      LimitWarning.checkList(groupObject['frames'], self.options.MAX_GROUP_FRAMES, 'frames in a group (group iid=' + str(groupObject['iid']) + ')')
    LimitWarning.checkList(objectionObject['groups'], self.options.MAX_GROUPS, 'groups')
    LimitWarning.checkList(objectionObject['pairs'], self.options.MAX_PAIRS, 'pairs')
...
class Case(_ObjectionBase):
  ...
  def compile(self):
    ...
    LimitWarning.checkList(objectionObject['courtRecord']['evidence'], self.options.MAX_EVIDENCE, 'evidence')
    LimitWarning.checkList(objectionObject['courtRecord']['profiles'], self.options.MAX_PROFILES, 'profiles')
...
class LimitWarning(Warning):
  ...
  def warn(cls, limit: int, limitTarget: str):
    warn(f'exceeded limit of {limit} {limitTarget} - objection.lol support is not guaranteed', LimitWarning)

  @classmethod
  def checkList(cls, lst: Sized, limit: Optional[int], limitTarget: str):
    if limit is not None and len(lst) > limit: cls.warn(limit, limitTarget)
